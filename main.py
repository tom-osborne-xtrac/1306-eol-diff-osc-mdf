...
def set_start(data):
    ...
    # File 1
    EoL_start_0 = np.argwhere(np.array(data['GC Test Phase'] == 110)).flatten().tolist()[0]
    print("START INDEX:", EoL_start_0, len(data))

    data = data[EoL_start_0:len(data)]
...
def set_ticks(test_type):
    match test_type:
        case "bedding":
            return {
                "x": (50, 10),             # major, minor
                "y0": (0, 500, 100, 50),     # lower limit, upper limit, major, minor
                "y1": (0, 2000, 500, 250),    # lower limit, upper limit, major, minor
                "y2": (-2500, 2500, 500, 250)    # lower limit, upper limit, major, minor
            }
        case "scen123":
            return {
                "x": (50, 10),              # major, minor
                ...
                "y2": (-160, 40, 20, 5)     # lower limit, upper limit, major, minor
            }
...
if config["show_IPTrq"]:
    ...
    set_axis([ax2], 'y', 'Torque [Nm]', 0, 500, 100, 50)
    ax2.legend(loc=1)
